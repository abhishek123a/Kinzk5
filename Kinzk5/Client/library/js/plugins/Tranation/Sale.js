...
function Save(e) {
    ...
    if (jj) {
        ...
        $('.order-list input[type="text"]').prop('required', false);
        ...
        columns: [
               {
                ...
                { data: "IntQty", "searchable": true, "width": "5%" },
                { data: "StrInvoiceNo", "searchable": true, "width": "4%" },
                { data: "DecSalecost", "searchable": true, "width": "5%" },
                { data: "DtSaleDate", render: function (data, type, full, meta) { return moment(data).format("DD/MM/YYYY"); } },
                {
                    data: "IntSaleId",
                    "render": function (data, type, full, meta) {
                        return '<a class="btn btn-outline btn-circle btn-sm blue" onclick="PrintData(' + "'ModeSql=" + "PrintInternal" + "&reportpath=Rept/Invoice.rdlc" + '&FromBranch=' + full.FromBranchId + '&InvoiceNo=' + full.StrInvoiceNo + '&spname=PlPrintSaleInvoice' + '&Clentid=' + full.IntClintId + "'" + ')"><i class="fa fa-print"></i>' + '</a>';
                    }
                },
        ]
    });
