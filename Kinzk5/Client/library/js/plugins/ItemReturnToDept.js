    ...
    r = function () {
        ...
        e.validate({
            ...
            messages: {
                ...
                ctl00$ContentPlaceHolder1$ddDepartment: {
                    CheckDropDownList: jQuery.validator.format("Select Department"),
...
function loadDept()
{
   var end = $('#ContentPlaceHolder1_ddunit').val();
    $.ajax({
        type: "POST",
        ...
        success: function (r) {
            ...
            $('#ReturnToDept').DataTable({
                "processing": true,
                "serverSide": false,
                lengthMenu: [[10, 20, 50, 100, 150, -1], [10, 20, 50, 100, 150, "All"]],
                pageLength: 50,
                ajax: {
                    url: '../Service/data.asmx/GetAllItemReturnByDept',
                    type: "POST",
                    ...
                    "data": {
                        ...
                        "Tobranch": $('#ContentPlaceHolder1_ddunit').val(),
                    }
                },
                columns: [
                       {
                           ...
                           'render': function (data, type, full, meta) {
                               return '<a onclick="Delete(' + data.IntDeptReturndId + "," + "'" + data.ItemName + "'" + ')" class="btn btn-outline btn-circle dark btn-sm red ' + data.Edit + '" > <i class="fa fa-trash-o"></i>' + '</a>'
                               +
                               '<a onclick="Edit(' + data.IntDeptReturndId + ')" class="btn btn-outline btn-circle btn-sm purple ' + data.Delete + '"> <i class="fa fa-edit"></i>' + '</a>';
                               return data;
                           },
...
function GetAvaibleItem(itemid) {
    ...
    var Deptid = $('#ContentPlaceHolder1_ddDepartment').val();
    $.ajax({
        url: '../Service/data.asmx/GetAvilableQtyDept',
        method: 'post',
        contentType: 'application/json;Charset=utf-8',
        data: "{branchid:'" + branchid + "',ItemId:'" + itemid + "',DeptId:'" + Deptid + "'}",
...
function Save(e) {
    ...
    _param.IntDeptReturnNo = $("#ContentPlaceHolder1_HIssueno").val();
    _param.DtReturnDate = $("#ContentPlaceHolder1_txtreturndate").val() == "" ? DmyToMdy(GetValue($("#ContentPlaceHolder1_txtreturndate"))) : "01/01/1900";
    _param.StrReturnBy = $("#ContentPlaceHolder1_txtname").val();
   
    var param_detail = JSON.stringify(_param);
    $.ajax({
        url: '../Service/WsItemReturnByDept.asmx/Save',
        method: 'post',
        contentType: 'application/json;Charset=utf-8',
