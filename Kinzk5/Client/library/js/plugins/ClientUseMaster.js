    ...
    r = function () {
        ...
        e.validate({
            ...
            messages: {
                ...
                ctl00$ContentPlaceHolder1$ddclint: {
                    CheckDropDownList: jQuery.validator.format("Select Client Name"),
                },
                ctl00$ContentPlaceHolder1$txtPass: {
...
function mycallbackfunc(v) {
    if (v.target.textContent == "Ok") {
        ...
        $("#ContentPlaceHolder1_ddclint").val('0').trigger('change');
...
function UpdateUser() {
    ...
    var param_detail = JSON.stringify(_param);
    $.ajax({
        url: '../Service/UserClient.asmx/Update',
        method: 'post',
        contentType: 'application/json;Charset=utf-8',
        data: '{model:' + param_detail + '}',
        success: function (data) {
